...
**`r totalNAs`** rows have missing step data.
...
The mean total number of steps taken per day is **`r format(meanStepsImpute, digits = 7, scientific = FALSE)`**.  
The median total number of steps taken per day is **`r format(medStepsImpute, digits = 7, scientific = FALSE)`**.
...
The new mean using the imputed values is **`r round(meanDiff, digits = 2)`** steps larger than the mean without imputed values.  
The new median using the imputed values is **`r round(medDiff, digits = 2)`** steps larger than the mean without imputed values.
...
First, calculate the steps per interval separately for weekdays vs. weekends. Then plot the time series.
